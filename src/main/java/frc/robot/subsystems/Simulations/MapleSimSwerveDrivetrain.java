...
public class MapleSimSwerveDrivetrain {
    ...
    public SwerveDriveState getState(Swerve drivetrain){

        SwerveModuleState[] moduleStates = new SwerveModuleState[simModules.length];
        SwerveModulePosition[] modulePositions = new SwerveModulePosition[simModules.length];
        

        for (int i = 0; i < simModules.length; i++) {
            moduleStates[i] = simModules[i].moduleSimulation.getCurrentState();
            modulePositions[i] = drivetrain.getModule(i).getPosition(false);
        }


        SwerveDriveState state = new SwerveDriveState();
        state.Pose = mapleSimDrive.getSimulatedDriveTrainPose();
        state.Speeds = mapleSimDrive.getDriveTrainSimulatedChassisSpeedsFieldRelative();
        state.ModuleStates = moduleStates;
        state.ModulePositions = modulePositions;
        state.RawHeading = state.Pose.getRotation();
        state.Timestamp = Utils.getCurrentTimeSeconds();
        state.OdometryPeriod = 1.0 / RobotMap.UPDATE_FREQ;
